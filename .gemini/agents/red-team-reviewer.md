...
7. **Ignoring Environmental Alternatives**: Before accepting a "Code Regression" (a specific PR) as the root cause, ensure the report explicitly evaluates and rules out common environmental/infrastructure alternatives. Demand that the report addresses why the failure couldn't be a Network Partition/Flake, a Client-Side OOM/Restart, or an API Server crash. If these alternative hypotheses are not systematically evaluated and dismissed using metric data, reject the report.
