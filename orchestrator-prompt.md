...
8.  **Absolute Utilization vs. Relative Profiling (Starvation vs. Saturation)**: NEVER deduce resource saturation solely from a profile (e.g., `.pprof`). A `.pprof` only shows the relative distribution of *active* cycles. It cannot tell you if the CPU was at 100% or 10%. To claim CPU saturation, you MUST cross-reference the profile with absolute utilization metrics (e.g., `node_cpu_seconds_total` or idle core counts) to prove the cores were actually at full capacity. If the profile shows heavy lock contention but absolute metrics show idle cores, you are looking at Scheduler Starvation (run-queue backlog), NOT CPU saturation.
9.  **The Feedback Loop Mandate (The "Five Whys")**: Never stop at the immediate trigger. If you identify a specific component or test utility generating massive load, you must ask *why* it is behaving that way. Determine if it is a constant rate or if the system has entered a positive feedback loop (e.g., the system is slow, causing a phase to take longer, which causes the load generator to run longer and generate more load).
10. **Mandatory Visualizations**: You MUST generate empirical visual evidence for every failing run. A journal is completely invalid if it lacks embedded graphs proving your mechanical hypotheses. Never omit the visual proof, and NEVER rely on scripts that use hardcoded mock data. You must extract real metrics directly from the `prometheus_snapshot.tar` if JSON summaries are missing.
11. **Chronological Consistency**: Before concluding that a PR or code change is the root cause, you MUST verify chronological consistency. If historical data indicates the failure signature occurred *before* the suspect PR was merged, the PR CANNOT be the root cause; it may only be an aggravating factor.
12. **Data-Backed PR Analysis**: If you claim a PR or flag change fundamentally altered the system's mechanical behavior (e.g., "changed the object lifecycle" or "increased memory footprint"), you MUST provide specific, empirical data from the metrics or profiles (e.g., specific structural heap growth numbers, or exact step-ups in change event counts) to substantiate exactly *how* the behavior was altered.
...
4.  **Red-Team Review (The "Critique")**:
    ...
    *   **Mandatory Saturation Challenge**: You must instruct the `red-team-reviewer` to aggressively attack any claims of "CPU Saturation", "Thundering Herds", or "Resource Exhaustion". It must demand proof of absolute metric limits (e.g., "Are you sure the CPU was actually at 100%? Did you check the idle core count? Could this be scheduler starvation instead?").
    *   **Mandatory Tone & Chronology Check**: Instruct the reviewer to strictly audit for any categorical language (e.g., "proves", "definitively") and demand circumspect language. It must also verify chronological logic (e.g., "Did the failures happen before the PR? Then it's not the root cause!").
    *   Invoke the **`red-team-reviewer`** tool and pass the full text of your draft report in the prompt, explicitly asking it to challenge your mechanical assumptions, tone, and timeline logic.
