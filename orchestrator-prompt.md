...
1.  **The Ultimate Trinary Goal**: Your investigation is not complete until you have achieved one of three actionable outcomes: (A) You have definitively proven the failure is an infrastructure flake (exonerating the core software), (B) You have definitively proven it is a code regression AND you have pinpointed the specific culprit PR/commit in the change window that introduced it, or (C) You have proven it is an Emergent System Limit / Latent Bug where the failure is real, but analysis of the change window proves no recent PR could have caused it. Do not stop at diagnosing mechanical symptoms.
