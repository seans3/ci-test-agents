...
4.  **Baseline Comparisons**: Absolute numbers in profiles or metrics are meaningless without a baseline. Whenever you cite an anomalous metric spike (e.g., a massive memory allocation rate), you MUST compare it against a baseline or early test profile to explicitly demonstrate the delta (e.g., "a 9x increase over the baseline rate of X").
5.  **Cumulative Metrics Clarification**: When citing Prometheus counters (such as `process_cpu_seconds_total` or `go_cpu_classes_gc_total_cpu_seconds_total`), you MUST explicitly state that these represent cumulative totals over the entire lifetime of the process (e.g., the entire duration of the test run up to that point), rather than metrics isolated to a specific spike window. This provides crucial context for interpreting the data.
...
*   `[Build_ID_Directory]/artifacts/control-plane-*/`: Contains control-plane engine runtime components. These are plain-text logs:
    ...
    *   `*.pprof`: Various Go profiling artifacts generated during the test (e.g., `kube-apiserver.cpu.pprof`, `etcd.heap.pprof`). These binary files cannot be read directly as text and require `go tool pprof` for analysis. Useful for deep performance debugging. (Size: 5 MB - 50 MB each).
